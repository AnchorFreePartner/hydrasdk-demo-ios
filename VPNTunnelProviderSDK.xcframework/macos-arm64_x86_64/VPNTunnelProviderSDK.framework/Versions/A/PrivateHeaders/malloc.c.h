...
#if DEBUG
#define LOG_PAGING_CHANGES
#endif

// #define MALLOC_ALIGNMENT ((size_t)16U)

#ifdef LOG_PAGING_CHANGES
void NSLog(CFStringRef format, ...);
#endif

volatile int use_os_x_malloc = 0;
volatile int malloc_uses_file_mmap = 0;
volatile char *mmap_base_dir = NULL;
volatile int32_t mmap_counter = 0;
volatile int64_t malloc_core_memory_limit = 0;
volatile int64_t malloc_core_memory_used = 0;


#define DARWIN
static inline void *af_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset);
static inline void *af_std_os_x_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset);

static inline void *af_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
#ifdef LOG_PAGING_CHANGES
    // NSLog(CFSTR("af_mmap called.\n"));
#endif
    
#if 1
    // if (use_os_x_malloc) {
    if (malloc_core_memory_limit && !malloc_uses_file_mmap) {
        bool logTransition = false;
        
        if (malloc_core_memory_used < malloc_core_memory_limit) {
            OSAtomicAdd64(len, &malloc_core_memory_used);
            logTransition = true;
        }
        
        if (malloc_core_memory_used >= malloc_core_memory_limit) {
            malloc_uses_file_mmap = 1;
            
#ifdef LOG_PAGING_CHANGES
            if (logTransition) NSLog(CFSTR("Enabling paging.\n"));
#endif
    ...
    //if (use_os_x_malloc) {
    //return af_std_os_x_malloc(bytes);
    //}
    //}
#endif
    
    char *retval = NULL;
    if (malloc_uses_file_mmap && !mmap_base_dir) {
        fprintf(stderr, "ERROR: malloc_uses_file_mmap = 1, but mmap_base_dir not yet set!\n");
        assert(false);
    }
    if (malloc_uses_file_mmap && mmap_base_dir) {
        char temp[MAXPATHLEN];

        snprintf(temp, sizeof(temp)-1, "%s/%d", mmap_base_dir, OSAtomicIncrement32(&mmap_counter));

        // printf("Mapped %s\n", temp);

        unlink(temp);
        int file_fd = open(temp, O_RDWR|O_CREAT, 0666);
        ftruncate(file_fd, len);

        retval = af_std_os_x_mmap(NULL, len, prot, MAP_SHARED|MAP_FILE, file_fd, offset);

        unlink(temp);
        close(file_fd);

    } else {
        retval = af_std_os_x_mmap(addr, len, prot, flags, fd, offset);
    }
    return retval;
}
...
static inline void *
af_std_os_x_mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset) {
    void *(*mmap_addr)(void *,size_t,int,int,int, off_t) = dlsym(RTLD_NEXT, "mmap");
    
    if (!mmap_addr) {
        mmap_addr = dlsym(RTLD_DEFAULT, "mmap");
        if (mmap_addr == mmap) {
            // NSLog(CFSTR("Nope.\n"));
            return MAP_FAILED;
        } else if (!mmap_addr) {
            // NSLog(CFSTR("Double nope.\n"));
            return MAP_FAILED;
        }
    }
    
    // NSLog(CFSTR("Our mmap: %p OS X mmap: %p\n"), mmap, mmap_addr);
    
    return mmap_addr(addr, len, prot, flags, fd, offset);
}

static inline void *
af_std_os_x_malloc(size_t size) {
	void *(*malloc_addr)(size_t) = dlsym(RTLD_NEXT, "malloc");
    
    if (!malloc_addr) {
        malloc_addr = dlsym(RTLD_DEFAULT, "malloc");
        if (malloc_addr == malloc) {
            // NSLog(CFSTR("Nope.\n"));
            return NULL;
        } else if (!malloc_addr) {
            // NSLog(CFSTR("Double nope.\n"));
	...
	// NSLog(CFSTR("Our malloc: %p OS X malloc: %p\n"), malloc, malloc_addr);

	return malloc_addr(size);
}


void af_std_os_x_free(void *buf) {
    void (*free_addr)(void *) = dlsym(RTLD_NEXT, "free");
    
    if (!free_addr) {
        free_addr = dlsym(RTLD_DEFAULT, "free");
        if (free_addr == free) {
            // NSLog(CFSTR("Nope.\n"));
            fprintf(stderr, "nedmalloc: system free failed [1]!\n");

            return;
        } else if (!free_addr) {
            fprintf(stderr, "nedmalloc: system free failed [2]!\n");
    ...
    // NSLog(CFSTR("Our free: %p OS X free: %p\n"), free, free_addr);
    
    free_addr(buf);
}

void *
af_std_os_x_realloc(void *ptr, size_t size)
{
void *(*realloc_addr)(void *, size_t) = dlsym(RTLD_NEXT, "realloc");

if (!realloc_addr) {
    realloc_addr = dlsym(RTLD_DEFAULT, "realloc");
    if (realloc_addr == realloc) {
        // NSLog(CFSTR("Nope.\n"));
        fprintf(stderr, "nedmalloc: system realloc failed [1]!\n");
        
        return NULL;
    } else if (!realloc_addr) {
        fprintf(stderr, "nedmalloc: system realloc failed [2]!\n");
...
{
    if (flags & MAP_ANON) {
        return af_mmap(addr, len, prot, (flags & ~MAP_ANON), fd, offset);
    }
    return af_std_os_x_mmap(addr, len, prot, flags, fd, offset);
}
...
#if __APPLE__
#undef MMAP_DEFAULT
#define MMAP_DEFAULT(s)       af_mmap(0, (s), MMAP_PROT, MMAP_FLAGS, -1, 0)
#undef DIRECT_MMAP_DEFAULT
#define DIRECT_MMAP_DEFAULT(h, s, f)        af_mmap(0, (s), MMAP_PROT, MMAP_FLAGS, -1, 0)
#endif
...
static uint32_t clz64(const uint64_t x)
{
    uint32_t u32 = (x >> 32);
...
{\
  ...
  if (X == 0)\
    ...
    size_t K = (size_t) sizeof(X)*__CHAR_BIT__ - 1 - (size_t) clz64(X); \
    I =  (bindex_t)((K << 1) + ((S >> (K + (TREEBIN_SHIFT-1)) & 1)));\
  }\
}
